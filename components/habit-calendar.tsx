    ...
    <div className={`bg-card rounded-xl p-4 ${className}`}>
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2 sm:gap-3">
          {/* Weekday labels */}
          <div className="text-muted-foreground flex flex-col gap-1 pb-1 text-xs">
            {weekdays.map((day, index) => (
              <div
                key={`weekday-${index}`}
                className="flex h-3 w-5 items-center justify-start sm:h-3 sm:w-6"
              >
                {day}
              </div>
            ))}
          ...
          <div className="flex-1 overflow-x-auto">
            ...
            >
              {calendarData.slice(-84).map((day, index) => (
                <div
                  key={`mobile-${index}`}
                  className={`h-3 w-3 rounded-sm border border-gray-300 ${getIntensity(
                    day.completionPercentage,
                    day.isInRange
                  )} ${day.isToday ? 'ring-primary ring-1 ring-offset-0' : ''}`}
